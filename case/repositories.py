...
class Repository:
    ...
    def __repr__(self) -> str:
        return "{class_name}[{cases}]".format(
            class_name=type(self).__name__, cases=", ".join(map(repr, self))
        )
...
class Cases(Repository):
    ...
    def get(self, style: str, default: typing.Any = None) -> typing.Any:
        items: typing.List[models.Case] = [case for case in self if case.style == style]

        return items[0] if items else default

    def identify(self, string: str) -> typing.List[models.Case]:
        return [case for case in self if case.match(string)]
